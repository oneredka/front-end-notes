...
/*,默认提交的数据都是get方式.还有一种post方式.
  ...
  get:表单数据会通过url发送(附加在地址栏上)对发送信息数量有限制
  post:表单数据会作为Http请求体一部分  
		地址栏看到不到提交信息,发送信息数量无限制



什么时候用get ,什么时候用post呢? 一般是这样来判断的，

如果只是为取得和显示数据,用get; 
一旦涉及数据的保存和更新，那么建议用post。
...
```css
<input type="image" src="./vodeo1.webp">/*以图片作为表单的提交按钮*/
```

### 隐藏域

```css
 <input type="hidden" name="hidden" value="这是一个隐藏信息">
/*用户无法看到  但是内容会提交给服务器*/
```
...
```css
<form action="">
		<select name="wife" size="2" >
			/* size列表可见的选项数目  multiple可以选择多个*/
...
form:focus-within{
    ...
    /*这算是目前唯一一个可以通过子元素影响父元素的css选择器了吧*/
}
<form action="">

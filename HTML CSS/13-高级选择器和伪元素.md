...
> **伪元素其实是凭空生成的一个元素,使用双冒号.创建不在文档树中的元素**
>
> **伪类:某种状态的原来的元素,使用单冒号,操作文档树中的元素**
>
...
<head>
	...
	<style>
		*{
		...
		.list li input[name="change"]:checked{
			width: 700px;
			height: 100px;
		}


		.list li input[name="change"] ~ .bg{
			opacity: 0;
			position: absolute;
			z-index: -5;
			top: 0;
			...
			height: 100%;
			transition: .5s;
			filter: blur(3px);/*每一个像素周围3个像素都会参与到颜色设置之中 取平均值*/
		}

		.list li input[name="change"]:checked ~ .bg{
			opacity: 1;
		}

		/* .list li:nth-child(1) input:checked ~ .bg{
			z-index: -4;
		}
		.list li:nth-child(2) input:checked ~.bg{
			z-index: -4;
		} */
	</style>
</head>
